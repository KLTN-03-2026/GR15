...
const displayName = computed(() => currentUser.value?.ho_ten || currentUser.value?.email || 'Quan tri vien')
...
const submitSearch = () => {
  ...
  if (keyword.includes('ho so') || keyword.includes('profile')) {
  ...
  if (keyword.includes('user skill') || keyword.includes('ky nang nguoi dung')) {
    router.push('/admin/user-skills')
    return
  }

  if (keyword.includes('skill') || keyword.includes('ky nang')) {
    ...
    <div class="flex flex-1 items-center gap-4">
      ...
      <form class="relative w-full max-w-xl" @submit.prevent="submitSearch">
        ...
        <input
          ...
          placeholder="Tim nhanh nguoi dung, cong ty, ho so, ky nang..."
          type="text"
        />
      </form>
    </div>
      ...
      <div ref="profileMenuRef" class="relative">
        ...
        >
          ...
          <div class="hidden min-w-0 sm:block">
            ...
            <p class="truncate text-xs uppercase tracking-[0.18em] text-slate-400">Quan tri he thong</p>
          </div>
        ...
        >
          <RouterLink
            to="/admin/stats"
            class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"
