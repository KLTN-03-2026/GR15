    ...
    <section class="overflow-hidden rounded-[30px] border px-8 py-8 shadow-[0_28px_90px_rgba(15,23,42,0.12)]" :class="statusTone">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p v-if="shouldAutoRedirectToBilling" class="mt-3 text-sm font-semibold opacity-80">
            Tự động quay về Ví Employer sau {{ redirectCountdown }} giây.
          </p>
        </div>
...
const successRedirectStorageKey = computed(() => `employer-payment-result-redirected:${paymentCode.value}`)
...
const heroDescription = computed(() => {
  if (paymentStatus.value === 'success') {
    return 'Số tiền đã được ghi nhận vào ví employer của bạn. Bạn có thể quay lại ví để xem số dư mới.'
  }
...
const shouldAutoRedirectToBilling = computed(() => paymentStatus.value === 'success' && !hasAutoRedirected.value)

const startSuccessRedirect = () => {
  clearRedirectTimer()

  if (!shouldAutoRedirectToBilling.value) return
  ...
  redirectTimer = window.setInterval(async () => {
    if (redirectCountdown.value <= 1) {
      ...
      await router.push('/employer/billing')
      return
    }
...
const loadPayment = async (options = {}) => {
  ...
  try {
    const response = await employerBillingService.getTopUp(paymentCode.value)
