<template>
  <div class="space-y-8">
    <section class="overflow-hidden rounded-[30px] border border-blue-200 bg-gradient-to-r from-slate-950 via-[#143c8c] to-[#2f7de1] px-8 py-8 text-white shadow-[0_28px_90px_rgba(37,99,235,0.2)]">
      <div class="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_340px] lg:items-end">
        <div>
          <p class="text-xs font-semibold uppercase tracking-[0.38em] text-blue-100/80">Payment History</p>
          ...
          <p class="mt-4 max-w-3xl text-base leading-8 text-blue-50/90">
            Theo dõi toàn bộ giao dịch nạp ví employer, kiểm tra trạng thái cuối cùng và mở chi tiết từng giao dịch khi cần.
          </p>
        </div>
        ...
        <div class="rounded-[24px] border border-white/15 bg-white/10 p-5 backdrop-blur">
          ...
          <p class="mt-2 text-sm leading-7 text-blue-50/85">
            {{ filters.loai_giao_dich || filters.trang_thai ? 'Danh sách đang được lọc theo tiêu chí bạn chọn.' : 'Bao gồm tất cả giao dịch nạp ví employer.' }}
          </p>
        </div>
      </div>
    </section>
    ...
    <div class="grid gap-5 md:grid-cols-4">
      <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <h2 class="mt-3 text-3xl font-black text-slate-900">{{ pagination.total }}</h2>
      </article>
      <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-sm font-medium text-slate-500">Nạp ví</p>
        <h2 class="mt-3 text-3xl font-black text-slate-900">{{ paymentStats.topups }}</h2>
      </article>
      <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-sm font-medium text-slate-500">Mua gói dịch vụ</p>
        <h2 class="mt-3 text-3xl font-black text-slate-900">{{ paymentStats.subscriptions }}</h2>
      </article>
      <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-sm font-medium text-slate-500">Đang chờ xác nhận</p>
        <h2 class="mt-3 text-3xl font-black text-amber-600">{{ paymentStats.pending }}</h2>
      </article>
    </div>
    ...
    <section class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      <div class="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-end md:justify-between">
        <div>
          <h2 class="text-2xl font-bold text-slate-900">Danh sách giao dịch</h2>
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
          <select
            ...
            class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-[#2463eb]"
          >
          ...
          <button
            type="button"
            class="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:opacity-60"
            :disabled="refreshing"
          ...
          <RouterLink
            :to="{ name: 'EmployerBilling' }"
            class="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
          >
      ...
      <div v-if="loading" class="mt-5 space-y-3">
        <div v-for="index in 6" :key="index" class="h-28 animate-pulse rounded-2xl bg-slate-100"></div>
      </div>

      <div v-else-if="!payments.length" class="mt-5 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center text-sm leading-7 text-slate-600">
          ...
          <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <p class="text-base font-bold text-slate-900">{{ getPaymentTitle(payment) }}</p>
            ...
            <div class="grid min-w-[220px] gap-2 text-sm">
              <div class="flex items-center justify-between rounded-xl bg-white px-3 py-2">
                ...
                <span class="font-bold text-slate-900">{{ formatCurrency(payment.so_tien) }}</span>
              </div>
              <div class="flex items-center justify-between rounded-xl bg-white px-3 py-2">
                <span class="text-slate-500">Gateway</span>
                <span class="font-bold text-slate-900">{{ getGatewayLabel(payment.gateway) }}</span>
              </div>
              <button
              ...
              <RouterLink
                :to="{ name: 'EmployerPaymentDetail', params: { maGiaoDichNoiBo: payment.ma_giao_dich_noi_bo } }"
                class="inline-flex items-center justify-center rounded-xl bg-slate-900 px-3 py-2 text-sm font-bold text-white transition hover:bg-slate-800"
              >
                Xem chi tiết
              </RouterLink>
            </div>
...
const getTypeLabel = (type) => {
  if (type === 'topup_wallet') return 'Nạp ví employer'
...
const getGatewayLabel = (gateway) => {
  ...
  if (gateway === 'wallet') return 'Ví Employer'
...
const loadPayments = async (page = pagination.current_page, silent = false) => {
  ...
  try {
    const response = await employerBillingService.getPayments({
      page,
