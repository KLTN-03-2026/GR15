      ...
      <aside class="space-y-4">
        ...
        <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_22px_60px_rgba(148,163,184,0.12)]">
          ...
          <ul class="mt-4 space-y-3 text-sm leading-7 text-slate-600">
            ...
            <li>3. Nếu giao dịch đã thành công nhưng giao diện cũ chưa cập nhật, hãy tải lại trang hoặc quay về màn hình tương ứng.</li>
          </ul>
        </div>
      </aside>
    </section>
...
const paymentTypeLabel = computed(() => {
  if (payment.value?.loai_giao_dich === 'topup_wallet') return 'Nạp ví employer'
  if (payment.value?.loai_giao_dich === 'buy_subscription') return 'Mua gói dịch vụ'
  return 'Thanh toán'
})
...
const heroDescription = computed(() => {
  return 'Theo dõi trạng thái nạp ví employer, mã giao dịch gateway và số dư ví liên quan tới giao dịch này.'
})
...
const loadPaymentDetail = async () => {
  loading.value = true
  try {
    const response = await employerBillingService.getPaymentDetail(paymentCode.value)
