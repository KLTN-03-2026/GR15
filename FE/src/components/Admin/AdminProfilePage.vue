...
import { emitAuthChanged, getStoredUser, updateStoredCandidate } from '@/utils/authStorage'
...
const normalizeProfile = (payload) => {
  ...
  return {
    ...
    vai_tro: profile.vai_tro ?? currentStoredUser.vai_tro ?? 2,
    ...
    ten_vai_tro: profile.ten_vai_tro ?? profile.role ?? 'Quản trị viên hệ thống',
...
const loadProfile = async () => {
  ...
  try {
    ...
    syncStoredAdmin(normalized)
  } catch (error) {
    setAlert('error', error.message || 'Không thể tải hồ sơ quản trị.', { autoDismiss: false })
  } finally {
...
const adminInitials = computed(() => {
  const words = (form.ho_ten || 'Quản trị viên')
    .trim()
    .split(/\s+/)
    .filter(Boolean)
...
const roleLabel = computed(() => {
  const role = adminProfile.value?.ten_vai_tro
...
const statusLabel = computed(() => {
  const status = adminProfile.value?.trang_thai
...
const createdAtLabel = computed(() => formatDate(adminProfile.value?.created_at))
      ...
      <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="bg-[radial-gradient(circle_at_top_right,_rgba(37,99,235,0.18),_transparent_36%),linear-gradient(135deg,#0f172a_0%,#1e293b_60%,#0f172a_100%)] px-6 pb-8 pt-7 text-white">
          <div class="mb-2 mt-3 flex items-start gap-4">
            ...
            <div class="min-w-0">
              <p class="truncate text-2xl font-black">{{ form.ho_ten || 'Quản trị viên' }}</p>
        ...
        <div class="space-y-4 p-5">
          ...
          <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
            <div class="flex items-center gap-2">
              ...
              <p class="text-sm font-bold text-slate-900 dark:text-slate-100">Phạm vi quản trị</p>
            </div>
            <p class="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
              Quản lý người dùng, phê duyệt dữ liệu và theo dõi vận hành hệ thống.
            </p>
          </div>
        </div>
      </section>
