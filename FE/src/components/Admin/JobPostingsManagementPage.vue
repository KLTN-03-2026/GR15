  ...
  >
    ...
    <div class="grid gap-5 lg:grid-cols-2">
      ...
      <div class="space-y-2">
        ...
        <input v-model="formData.kinh_nghiem_yeu_cau" type="text" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-base text-slate-900 outline-none transition focus:border-[#2463eb] focus:bg-white focus:ring-2 focus:ring-[#2463eb]/20" placeholder="Ví dụ: 6 tháng, 0.5, 1 năm" />
        <p class="text-xs text-slate-500">Có thể nhập theo tháng như CV; ví dụ 6 tháng sẽ được tính là 0.5 năm khi matching.</p>
      </div>
      <div class="space-y-2">
...
const submitEdit = async () => {
  ...
  try {
    await adminJobPostingService.updateJob(editingJob.value.id, {
      ...
      kinh_nghiem_yeu_cau: normalizeExperienceRequirementText(formData.kinh_nghiem_yeu_cau) || null,
